...
1. agent 自己在目标技能项目目录里启动 `cli-skill browser record`
2. 直接打开录制浏览器，让用户在浏览器里完成真实操作
3. 持续等待录制命令结束，不要把命令抛给用户自己执行
4. 录制结束后先分析结果
5. 再决定沉淀成一个工具还是多个工具
6. 最后才生成工具
...
4. 只在需要更多上下文时，再按时间线里的 id 去查明细
   同时注意事件里的 `pageId`，不要把不同页面的后续变化读成同一页
5. 先判断失败更像：
   - 选择器问题
   - 页面时序问题
   - 登录态问题
...
2. 直接让录制浏览器进入前台，等待用户完成操作
3. 持续等待录制命令结束
4. 先读 `summary.json`
5. 再读 `timeline.jsonl`
6. 需要更多上下文时，再按时间线里的 `actionId`、`networkId`、`domSnapshotId` 去查：
   - `actions.jsonl`
   - `network.jsonl`
   - `dom.jsonl`
   并结合 `pageId` 判断这些记录属于哪个页面
7. 先写“用户做了什么”
8. 再判断适合沉淀成哪些工具
9. 再实现工具和技能文档
...
- `cli-skill browser record` 是一个长任务：
  ...
  - agent 启动后必须持续等待这个进程退出
  - 不要默认把命令输出给用户，让用户自己回到终端执行
